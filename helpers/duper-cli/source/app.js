    ...
    if (input === 's') {
      ...
      for (let d of dupes) {
        for (let filePath of d.filePaths) {
          commandString += `${toSave.includes(filePath) ? '# ' : ''}rm -v '${filePath.replace(/\'/g, `\\'`)}';\n`
        }
        commandString += '\n'
      }
      ...
      exit(0)
		...
		if ((input === 'b' || key.leftArrow) && canMoveBack) setDupeIndex((prev) => prev - 1)
		else if ((input === 'n' || key.rightArrow) && canMoveForward) setDupeIndex((prev) => prev + 1)
		else if (nonZeroNumbers.includes(input)) {
      const filePath = dupe.filePaths[parseInt(input, 10) - 1]
      if (!filePath) return
      if (toSave.includes(filePath)) setToSave((prev) => prev.filter((p) => p !== filePath))
    ...
    <Box borderStyle="round" width={78} paddingLeft={1} paddingRight={1}>
      <Box justifyContent="center" alignItems="center">
        <Text>
          ...
          {'    '}
						<Text bold dimColor={!canMoveBack} color="green">← b</Text>ack{'    '}
						<Text bold dimColor={!canMoveForward} color="green">n</Text>ext <Text bold dimColor={!canMoveForward} color="green">→</Text>
          {'    '}
          <Text bold color="green">s</Text>ave and quit{'    '}<Text bold color="green">q</Text>uit
        </Text>
      </Box>
    </Box>
    <Text>
      {
        dupe.filePaths.map((p, idx) => <Text color={toSave.includes(p) ? 'green' : 'red'} key={p}><Text bold>{idx + 1}</Text>. {p}<Newline /></Text>)
      }
    </Text>
  </Box>
}
